...
import 'package:dart_orm/src/dmmf/mappings.dart';
import 'package:dart_orm/generator_helper.dart' as gh;
  ...
  Class generateClient() {
    return Class((builder) {
      ...
      builder.fields.add(Field((builder) {
        ...
        builder.type = refer('TransactionClient<PrismaClient>?', 'package:dart_orm/dart_orm.dart');
      }));
...
extension on Generator {
  Method createTransactionGetter() {
    return Method((getter) {
      ...
      getter.body = Block.of([
        ...
        refer('TransactionClient<PrismaClient>', 'package:dart_orm/dart_orm.dart').code,
        Code('transaction) {'),
        Code('''
  ...
  Method createEngineGetter() {
    ...
    final datasources = options.datasources.map((e) {
      final (value, type) = switch (e.url) {
        gh.EnvVar(name: final String name) => (name, 'DatasourceType.environment'),
        gh.EnvValue(value: final String url) => (url, 'DatasourceType.url')
      };

      return MapEntry(e.name, refer('Datasource', 'package:dart_orm/dart_orm.dart').newInstance([refer(type, 'package:dart_orm/dart_orm.dart'), literalString(value)]));
    });
    ...
    return Method((builder) {
      ...
      builder.body = refer('_engine').assignNullAware(engine.newInstance([], {
  ...
  Field generateDatamodel() {
    return Field((builder) {
      ...
      builder.assignment = refer('DataModel', 'package:dart_orm/dmmf.dart').newInstanceNamed('fromJson', [
...
final _defaultConstructor = Constructor((builder) {
  ...
  const superOptionalParameters = ['datasourceUrl', 'datasources', 'errorFormat', 'log'];
  ...
  builder.optionalParameters.add(Parameter((builder) {
    builder.name = 'engine';
    builder.named = true;
    builder.type = Reference('Engine?', 'package:dart_orm/dart_orm.dart');
  }));
...
final _engineField = Field((builder) {
  ...
  builder.type = refer('Engine?', 'package:dart_orm/dart_orm.dart');
});
