...
extension GenerateEnum on Generator {
  Reference generateEnum(String name, dmmf.TypeNamespace? namespace, bool isList) {
    if (name.endsWith('ScalarFieldEnum') && namespace == dmmf.TypeNamespace.prisma) {
...
extension on Generator {
  dmmf.Enum findEnum(String name, dmmf.TypeNamespace? namespace) {
    ...
    return enums.firstWhere((element) => element.name == name, orElse: () => throw ArgumentError('Enum $name not found in namespace ${namespace?.name}.', name));
  }
}
