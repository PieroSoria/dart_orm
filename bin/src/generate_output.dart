...
import 'package:dart_orm/dmmf.dart' as dmmf;
...
extension GenerateOutput on Generator {
  Reference generateOutput(String type, dmmf.TypeNamespace? namespace, bool isList) {
...
extension on Generator {
  Method generateToJsonMethod(dmmf.OutputType output) {
    Expression generateValueExpression(Expression value, dmmf.OutputField field, [bool nullable = true]) {
      if (field.outputType.location == dmmf.TypeLocation.outputObjectTypes) {
        final call = nullable ? value.nullSafeProperty('toJson') : value.property('toJson');

        return call([]);
      } else if (field.outputType.location == dmmf.TypeLocation.scalar && field.outputType.type == 'DateTime') {
        final call = nullable ? value.nullSafeProperty('toIso8601String') : value.property('toIso8601String');
        return call([]);
      } else if (field.outputType.location == dmmf.TypeLocation.enumTypes) {
        return nullable ? value.nullSafeProperty('name') : value.property('name');
      }
  ...
  Constructor generateFromJsonConstructor(dmmf.OutputType output) {
    return Constructor((builder) {
      ...
      builder.lambda = true;
      builder.body = refer(output.name.className).newInstance([], generateFromJsonFields(output)).code;
    });
  }
  ...
  Map<String, Expression> generateFromJsonFields(dmmf.OutputType output) {
    ...
    for (final field in output.fields) {
      final value = switch (field.outputType.location) {
        ...
        dmmf.TypeLocation.outputObjectTypes => generateFromJsonOutputField(field),
  ...
  Expression generageOtherField(dmmf.OutputField field) {
    ...
    if (field.outputType.type == 'DateTime') {
      return CodeExpression(Block.of([
        Code('switch ('), type.code, Code(') {'),
        Code('DateTime value => value,'),
        Code('String value => DateTime.parse(value),'),
        Code('_ =>'), type.code, Code('}')
      ]));
    }

    if (!field.outputType.isList) return type;

    final when = type.asA(refer('Iterable')).property('whereType').call([], {}, [field.outputType.type.scalar]);
  ...
  Expression generateFromJsonOutputField(dmmf.OutputField field) {
    ...
    final deserialize = generateType(field.outputType).newInstanceNamed('fromJson', [refer('json').index(literalString(field.name))]);
  ...
  Expression generateFromJsonOutputListField(dmmf.OutputField field) {
    final type = generateType(dmmf.TypeReference(isList: false, location: field.outputType.location, namespace: field.outputType.namespace, type: field.outputType.type));
    ...
    return refer('json').index(literalString(field.name)).asA(TypeReference((type) {
      type.symbol = 'Iterable';
      type.isNullable = true;
    })).nullSafeProperty('map').call([fromJson.closure]);
  }
  ...
  Expression generateFromJsonEnumField(dmmf.OutputField field) {
    Method generateClosure(String parameterName, Expression value) {
      return Method((builder) {
        ...
        builder.body = refer(parameterName).property('name').equalTo(value).code;
      });
    }
    ...
    final enum$ = generateEnum(field.outputType.type, field.outputType.namespace, false);
    final value = refer('json').index(literalString(field.name));
    ...
    if (field.outputType.isList) {
      final wrapper = Method((builder) {
        ...
        builder.body = firstWhereCall(enum$, generateClosure('e', refer('child')).closure).code;
      });
      final deserialize = value.asA(refer('Iterable')).property('map').call([wrapper.closure]);

      return makeReurns(value, deserialize);
    }

    return makeReurns(value, firstWhereCall(enum$, generateClosure('e', value).closure));
  }
  ...
  dmmf.OutputType findOutputByNamespace(String name, dmmf.TypeNamespace? namespace) {
    ...
    return outputs.firstWhere((element) => element.name == name, orElse: () => throw ArgumentError('OutputType $name not found in namespace ${namespace?.name}.', name));
  }
}
