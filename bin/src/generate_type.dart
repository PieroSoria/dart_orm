...
extension GenerateType on Generator {
  Reference generateType(dmmf.TypeReference type, {bool? isList}) {
    return switch (type.location) {
      dmmf.TypeLocation.inputObjectTypes => generateInput(type.type, type.namespace, isList ?? type.isList),
      dmmf.TypeLocation.outputObjectTypes => generateOutput(type.type, type.namespace, isList ?? type.isList),
      dmmf.TypeLocation.enumTypes => generateEnum(type.type, type.namespace, isList ?? type.isList),
      dmmf.TypeLocation.fieldRefTypes => generateFieldRef(type.type).list(isList ?? type.isList),
