...
extension GenerateModelScalar on Generator {
  Reference generateModelScalar(String enumName) {
    final modelName = enumName.substring(0, enumName.indexOf('ScalarFieldEnum'));
    final name = '${modelName}Scalar'.className;
    if (generated.prisma.contains(name)) {
      return refer(name);
    }

    final enum$ = options.dmmf.schema.enumTypes.prisma.firstWhere((element) => element.name == enumName, orElse: () => throw ArgumentError('EnumType $enumName not found in namespace prisma.', enumName));
    ...
    libraries.prisma.body.add(Enum((builder) {
      ...
      builder.implements.addAll([
        refer('PrismaEnum', 'package:dart_orm/dart_orm.dart'),
      ...
      for (final field in fields) {
        builder.values.add(EnumValue((builder) {
          ...
          builder.arguments.addAll([literalString(field.name), literalString(model.name)]);

          final type = switch (field.kind) {
            dmmf.FieldKind.scalar => field.type.scalar,
            _ => generateType(dmmf.TypeReference(isList: field.isList, namespace: dmmf.TypeNamespace.model, location: dmmf.TypeLocation.enumTypes, type: field.type))
          };
...
extension on Generator {
  dmmf.Model findModel(String name) {
    return options.dmmf.datamodel.models.firstWhere((element) => element.name.toLowerCase() == name.toLowerCase(), orElse: () => throw ArgumentError('Model $name not found.', name));
  }
}
