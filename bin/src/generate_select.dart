...
extension GenerateSelect on Generator {
  Reference generateSelect(dmmf.OutputField field) {
    ...
    libraries.prisma.body.add(Class((builder) {
      ...
      builder.implements.add(TypeReference((builder) {
        ...
        builder.url = 'package:dart_orm/dart_orm.dart';
  ...
  Reference generateTypeWithFieldArgs(dmmf.OutputField field, String outoutName) {
    final name = '${outoutName.className}${field.name.className}Args';
    if (generated.prisma.contains(name)) {
      return TypeReference((builder) {
        builder.symbol = 'PrismaUnion';
        builder.url = 'package:dart_orm/dart_orm.dart';
        builder.types.addAll([refer('bool'), refer(name).namespace(dmmf.TypeNamespace.prisma)]);
      });
    }
    ...
    if (field.outputType.location == dmmf.TypeLocation.outputObjectTypes) {
      ...
      fields.add(dmmf.InputField(
        ...
        isNullable: true,
        isRequired: false,
        inputTypes: [dmmf.TypeReference(isList: false, namespace: dmmf.TypeNamespace.prisma, location: dmmf.TypeLocation.outputObjectTypes, type: '${field.outputType.type}Select')],
      ));
    }
    ...
    if (allowInclude(field.outputType)) {
      ...
      fields.add(dmmf.InputField(
        ...
        isNullable: true,
        isRequired: false,
        inputTypes: [dmmf.TypeReference(isList: false, namespace: dmmf.TypeNamespace.prisma, location: dmmf.TypeLocation.outputObjectTypes, type: '${field.outputType.type}Include')],
      ));
    }

    final input = dmmf.InputType(name: name, constraints: const dmmf.InputTypeConstraints(), fields: fields);
    final type = generateInputByInput(input, namespace: dmmf.TypeNamespace.prisma);
...
extension on Generator {
  Method generateToJson(dmmf.OutputType output) {
    final entries = output.fields.map((e) => MapEntry(e.name, refer(e.name.propertyName)));
  ...
  dmmf.OutputType findOutputField(dmmf.TypeReference type) {
    ...
    return types.firstWhere((element) => element.name == type.type, orElse: () => throw ArgumentError('Output type ${type.type} not found.', type.type));
  }
}
