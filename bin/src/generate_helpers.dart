...
extension GenerateHelpers on Generator {
  ...
  Reference generateUnionType(Iterable<dmmf.TypeReference> types) {
    ...
    return TypeReference((builder) {
      ...
      builder.types.addAll([generateType(types.first), generateUnionType(types.skip(1))]);
    });
  }

  bool allowSelect(dmmf.ModelAction action) => !_disallowedSelectActions.contains(action);
  ...
  bool allowInclude(dmmf.TypeReference type) {
    ...
    final output = types.firstWhereOrNull((element) => element.name == type.type);
    final modelRefs = output?.fields.where((element) => models.contains(element.outputType.type));
