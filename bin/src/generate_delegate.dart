...
extension on Generator {
  Method generateAction((dmmf.ModelAction, String) action, String model) {
    ...
    final returnsType = generateType(field.outputType).nullable(field.name.endsWith('OrThrow') ? false : field.isNullable);
    final returns = TypeReference((type) {
      type.symbol = 'ActionClient';
      type.types.add(returnsType);
      type.url = 'package:dart_orm/dart_orm.dart';
    });
    ...
    return Method((method) {
      ...
      method.body = Block.of([
        ...
        generateResult(),
        returns.newInstance([], {'action': literalString(action.$2), 'result': refer('result'), 'factory': generateFactory(field)}).returned.statement,
      ]);
    });
  }

  Expression generateFactory(dmmf.OutputField field) {
    final serialize = generateType(field.outputType, isList: false).property('fromJson');
    ...
    if (field.outputType.isList) {
      return Method((builder) {
        ...
        final mapProperty = switch (field.isNullable) {
          true => refer('values').asA(refer('Iterable?')).nullSafeProperty('map'),
    ...
    return Method((builder) {
      ...
      if (field.isNullable && !orThrow) {
        builder.body = refer('e').notEqualTo(literalNull).conditional(thenTrue, literalNull).code;
      }
    }).closure;
  }

  Code generateResult() {
    final request = refer('_client').property('\$engine').property('request').call([refer('query')], {
      'headers': refer('_client').property('\$transaction').property('headers'),
      'transaction': refer('_client').property('\$transaction').property('transaction'),
    });
  ...
  Code generateQuery((dmmf.ModelAction, String) action, String model) {
    final args = <String, Expression>{
      ...
      'action': refer('JsonQueryAction', 'package:dart_orm/dart_orm.dart').property(action.$1.toJsonQueryAction().name),
      'datamodel': refer('PrismaClient').property('datamodel'),
    };
    final serialize = refer('serializeJsonQuery', 'package:dart_orm/dart_orm.dart').call([], args);
  ...
  Code generateArgs(dmmf.OutputField field, dmmf.ModelAction action) {
    final args = Map.fromEntries(field.args.map((e) {
      if (action == dmmf.ModelAction.groupBy && e.name == 'by') {
        return MapEntry('by', refer('JsonQuery', 'package:dart_orm/dart_orm.dart').property('groupBySerializer').call([refer('by')]));
      }
    ...
    final select = switch (action == dmmf.ModelAction.groupBy) {
      true => refer('select').ifNullThen(refer('JsonQuery', 'package:dart_orm/dart_orm.dart').property('groupBySelectSerializer').call([refer('by')])),
      _ => refer('select'),
    };

    return declareFinal('args').assign(literalMap({...args, if (allowSelect(action)) 'select': select, if (allowInclude(field.outputType)) 'include': refer('include')})).statement;
  }
  ...
  dmmf.OutputField findActionDefinition(String action) {
    return options.dmmf.schema.outputTypes.prisma
        .where((element) => element.name == 'Query' || element.name == 'Mutation')
        .map((e) => e.fields)
        .expand((element) => element)
        .firstWhere((element) => element.name == action, orElse: () => throw Exception('Not found action $action in schema'));
  }
}
