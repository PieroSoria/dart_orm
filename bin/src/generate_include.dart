...
extension on Generator {
  Method generateToJson(dmmf.OutputType output) {
    final entries = output.fields.where(isModuleRef).map((e) => MapEntry(e.name, refer(e.name.propertyName)));
  ...
  Field generateField(dmmf.OutputField field, String outoutName) {
    return Field((builder) {
      ...
      builder.type = generateTypeWithFieldArgs(field, outoutName).nullable(true);
    });
  }
  ...
  dmmf.OutputType findOutputField(dmmf.TypeReference type) {
    ...
    return types.firstWhere((element) => element.name == type.type, orElse: () => throw ArgumentError('OutputType ${type.type} not found in namespace ${type.namespace}.', type.type));
  }
}
