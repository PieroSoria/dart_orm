...
extension GenerateInput on Generator {
  Reference generateInput(String name, dmmf.TypeNamespace? namespace, bool isList) {
    final input = findInputByNamespace(name, namespace);
    return generateInputByInput(input, namespace: namespace).list(isList);
  }

  Reference generateInputByInput(dmmf.InputType input, {dmmf.TypeNamespace? namespace}) {
...
extension on Generator {
  Method generateToJson(dmmf.InputType input) {
    final entries = input.fields.map((e) => MapEntry(e.name, refer(e.name.propertyName)));
  ...
  dmmf.InputType findInputByNamespace(String name, dmmf.TypeNamespace? namespace) {
    ...
    return types.firstWhere((e) => e.name == name, orElse: () => throw ArgumentError('InputType $name not found in namespace ${namespace?.name}.', name));
  }
}
