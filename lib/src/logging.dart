...
final class LogEvent extends EngineEvent {
  ...
  const LogEvent({required super.timestamp, required super.target, required this.message});
}
...
final class QueryEvent extends EngineEvent {
  ...
  const QueryEvent({required super.timestamp, required super.target, required this.duration, required this.params, required this.query});
}
...
class LogEmitter {
  ...
  LogEmitter(LogDefinition definition) : definition = LogDefinition.unmodifiable(definition);
  ...
  void _emitStdout(LogLevel level, EngineEvent event) {
    if (definition.should(level, LogEmit.stdout)) {
      ...
      final message = switch (event) {
        ...
        QueryEvent(query: final String query, params: final String params) => '$query - $params',
