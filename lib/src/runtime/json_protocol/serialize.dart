...
extension ModelActionToJsonQueryAction on dmmf.ModelAction {
  JsonQueryAction toJsonQueryAction() {
    return switch (this) {
      ...
      dmmf.ModelAction.createManyAndReturn => JsonQueryAction.createManyAndReturn,
      dmmf.ModelAction.update => JsonQueryAction.updateOne,
      dmmf.ModelAction.updateMany => JsonQueryAction.updateMany,
      dmmf.ModelAction.updateManyAndReturn => JsonQueryAction.updateManyAndReturn,
...
extension<K, V> on Map<K, V> {
  Map<K, V> withoutKeys(Iterable<K> keys) => Map.from(this)..removeWhere((k, _) => keys.contains(k));
...
class _Context {
  ...
  const _Context({required this.datamodel, required this.action, this.modelName, this.arguments = const [], this.selections = const []});

  _Context nestArgument(String argument) {
    return _Context(datamodel: datamodel, modelName: modelName, action: action, arguments: [...arguments, argument], selections: selections);
  }

  _Context nestSelection(String selection) {
    return _Context(datamodel: datamodel, modelName: modelName, action: action, arguments: arguments, selections: [...selections, selection]);
  }
  ...
  dmmf.Model? get model => datamodel.models.where((element) => element.name == modelName).firstOrNull;
...
JsonQuery serializeJsonQuery({required dmmf.DataModel datamodel, required JsonQueryAction action, String? modelName, Map? args}) {
  final context = _Context(datamodel: datamodel, modelName: modelName, action: action);

  return JsonQuery(action: action, modelName: modelName, query: _serializeFieldSelection(context, args ?? const {}));
}

Map<String, dynamic> _serializeFieldSelection(_Context context, [Map args = const {}]) {
  final Map<String, dynamic> select = switch (JsonConvertible.serialize(args['select'])) {
    Map select => select.cast(),
    _ => const <String, dynamic>{},
  };
  final Map<String, dynamic> include = switch (JsonConvertible.serialize(args['include'])) {
    Map include => include.cast(),
    _ => const <String, dynamic>{},
  };

  final arguments = _serializeArgumentsObject(context, args.withoutKeys(['select', 'include']));
  final selection = _serializeSelection(context, select, include);

  return {'arguments': _nullsSerialize(arguments), 'selection': _nullsSerialize(selection)};
}

Map<String, dynamic> _serializeSelection(_Context context, Map<String, dynamic> select, Map<String, dynamic> include) {
  if (select.isNotEmpty && include.isNotEmpty) {
    throw context.throwValidationError('Can not use `select` and `include` at the same time');
  }

  return select.isNotEmpty ? _createExplicitSelection(context, select) : _createImplicitSelection(context, include);
}

Map<String, dynamic> _createImplicitSelection(_Context context, Map<String, dynamic> include) {
  final fill = context.model != null && !context.isRawAction();
  final result = <String, dynamic>{if (fill) r'$composites': true, if (fill) r'$scalars': true};

  return {...result, ..._createIncludeRelations(context, include)};
}

Map<String, dynamic> _createIncludeRelations(_Context context, Map<String, dynamic> include) {
  final result = include.map((key, value) {
    final field = context.findField(key);
    if (field != null && field.kind != dmmf.FieldKind.object) {
      throw context.throwValidationError('Can not include field `$key` because it is not a relation');
    }
...
Map<String, dynamic> _createExplicitSelection(_Context context, Map<String, dynamic> select) {
...
Map<String, dynamic> _serializeArgumentsObject(_Context context, Map args) {
  ...
  return args.withoutNulls().map((key, value) {
    return MapEntry(key, _serializeArgumentValue(context.nestArgument(key.toString()), value));
  });
}

Map<String, dynamic> _createTypedValue(String type, dynamic value) => {r'$type': type, r'value': value};
...
dynamic _serializeArgumentValue(_Context context, dynamic value) {
  return switch (value) {
    ...
    DateTime value => _createTypedValue('DateTime', value.toUtc().toIso8601String()),
    ...
    Reference ref => _createTypedValue('FieldRef', {'_container': ref.model, '_ref': ref.name}),
    Decimal value => _createTypedValue('Decimal', value.toString()),
    PrismaEnum value => _createTypedValue('Enum', value.name),
    PrismaJson(value: final value) => _createTypedValue('Json', json.encode(value)),
    ...
    _ => context.throwValidationError('Argument value `$value` is not supported'),
  };
}
...
Iterable _serializeIterable(_Context context, Iterable iterable) {
  return iterable.indexed.map((element) {
    ...
    if (element.$2 == null) {
      throw context.throwValidationError('Can not serialize iterable with null value, If you want to serialize null value, use `PrismaNull` instead');
    }
