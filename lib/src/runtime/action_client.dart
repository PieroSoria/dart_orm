...
class ActionClient<T> implements Future<T> {
  ...
  const ActionClient({required String action, required Future<Map> result, required T Function(dynamic unserialized) factory})
      : _action = action, _result = result, _factory = factory;

  Future<dynamic> unserialized() => Future.value(_result).then((value) => value[_action]);
  ...
  Future<T> timeout(Duration timeLimit, {FutureOr<T> Function()? onTimeout}) {
    return unserialized().then(_factory).timeout(timeLimit, onTimeout: onTimeout);
  }
