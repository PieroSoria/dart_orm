...
String serializeRawParams(Iterable<Object?> params) {
  return json.encode(params.map(_encode).toList(growable: false));
}
...
Object? _encode(Object? value) {
  return switch (value) {
    ...
    DateTime value => _createTypedValue('date', value.toUtc().toIso8601String()),
    ...
    Map value => value.map((key, value) => MapEntry(key.toString(), _encode(value))),
    _ => value,
  };
}

Map<String, Object> _createTypedValue(String type, Object value) {
  return {'prisma__type': type, 'prisma__value': value};
}
