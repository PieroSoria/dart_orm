...
abstract class BasePrismaClient<Client extends BasePrismaClient<Client>> {
  BasePrismaClient({String? datasourceUrl, final Map<String, String>? datasources, ErrorFormat errorFormat = ErrorFormat.colorless, LogDefinition? log})
      : _options = PrismaClientOptions(datasources: datasources, datasourceUrl: datasourceUrl, errorFormat: errorFormat, logEmitter: LogEmitter(log ?? const {}));
  ...
  void $on(LogLevel level, void Function(EngineEvent event) listener) => $options.logEmitter.on(level, listener);
}
