...
class GeneratorApp {
  ...
  factory GeneratorApp.channel({required Stream<String> input, required StreamSink<String> output}) {
    return GeneratorApp(Server(StreamChannel<String>(input, output)));
  }

  factory GeneratorApp.stdio({required Stdin stdin, required Stdout stdout}) {
  ...
  void onManifest(FutureOr<GeneratorManifest?> Function(GeneratorConfig config) handler) {
  ...
  void onGenerate<T>(FutureOr<T> Function(GeneratorOptions options) handler) {
