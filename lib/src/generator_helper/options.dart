...
class GeneratorOptions {
  ...
  const GeneratorOptions({required this.generator, required this.otherGenerators, required this.schemaPath, required this.schema, required this.dmmf, required this.datasources, required this.version, required this.binaryPaths, this.postinstall = false, this.noEngine = false});

  factory GeneratorOptions.fromJson(Map json) {
    return GeneratorOptions(
      generator: GeneratorConfig.fromJson(json['generator']),
      otherGenerators: (json['otherGenerators'] as Iterable).map((e) => GeneratorConfig.fromJson(e)),
      schemaPath: json['schemaPath'],
      schema: json['schema'] ?? json['datamodel'],
      dmmf: DMMF.fromJson(json['dmmf']),
      datasources: (json['datasources'] as Iterable).map((e) => Datasource.fromJson(e)),
...
class Datasource {
  ...
  const Datasource({required this.name, required this.provider, required this.activeProvider, required this.url, this.directUrl, required this.schemas});
  ...
  factory Datasource.fromJson(Map json) {
    return Datasource(
      ...
      directUrl: json['directUrl'] != null ? Env.fromJson(json['directUrl']) : null,
...
enum ConnectorType {
  ...
  static ConnectorType of(String name) {
    return switch (name) {
      ...
      _ => throw ArgumentError.value(name, 'name', 'Invalid connector type, must be one of: ${ConnectorType.values.join(', ')}.'),
    };
  }
}
